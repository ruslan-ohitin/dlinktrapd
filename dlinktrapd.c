...
int get_quantity_of_ports(char *model)
{
  ...
  for (;;)
  {
    if (!strlen(model_name[i]))
      return (0);
    if (!strcasecmp(model, model_name[i]))
      break;
    i++;
  }
...
void switch_state(int pktsize)
{
  ...
  if (debug)
    printf("Model: %s, %i ports, code of message: %i, clear message: %s\n", model, qports, msgCode, msgClear);
...
int main(int argc, char *argv[])
{
...
#else
  if ((pTemp = strrchr(argv[0], '/')) != NULL)
    strlcpy(progname, pTemp + 1, sizeof(progname));
  else
  ...
  re_main = pcre_compile(pattern_main, 1, &error, &erroffset, NULL);
  if (re_main == NULL)
  {
  ...
  if (savestate)
  {
    ...
    if (!strlen(statedir))
      strlcpy(statedir, STATEDIR, sizeof(statedir));
